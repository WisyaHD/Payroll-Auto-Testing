...
import { alamat, jenisBayarDebit, kodeSales, namaCustomer, noHp, noRekening, password, username } from '../../../../constant';
...
describe('TransaksiPenjualanCashDebit', () => {
    ...
    it('click menu', () => {
        ...
        cy.wait(1000);
        page.penjualanPage.transaksiPenjualanPage.modalDataBarang.KodeBarcodeInputBox().type('00002266');
        cy.wait(5000);
        page.penjualanPage.transaksiPenjualanPage.modalDataBarang.hargaJualInputBox().clear();
        page.penjualanPage.transaksiPenjualanPage.modalDataBarang.hargaJualInputBox().type('500000');
        cy.wait(1000);
        page.penjualanPage.transaksiPenjualanPage.modalDataBarang.SimpananDataButtonBox().click({force: true});
        page.modalOtorisasi.usernameInputBox().type('helpdesk');
        page.modalOtorisasi.passwordInputBox().type(`${password}`);
        page.modalOtorisasi.keteranganInputBox().type('ubah harga jual');
